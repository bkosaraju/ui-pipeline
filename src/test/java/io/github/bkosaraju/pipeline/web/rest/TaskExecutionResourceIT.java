...
public class TaskExecutionResourceIT {
    ...
    private static final ZonedDateTime SMALLER_TASK_EXECUTION_TIMESTAMP = ZonedDateTime.ofInstant(
    ...
    public void getAllTaskExecutions() throws Exception {
        ...
        restTaskExecutionMockMvc
            .perform(get("/api/task-executions?sort=id,desc"))
    ...
    public void getTaskExecution() throws Exception {
        ...
        restTaskExecutionMockMvc
            .perform(get("/api/task-executions/{id}", taskExecution.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getAllTaskExecutionsByTaskExecutionTimestampIsInShouldWork() throws Exception {
        ...
        defaultTaskExecutionShouldBeFound(
            "taskExecutionTimestamp.in=" + DEFAULT_TASK_EXECUTION_TIMESTAMP + "," + UPDATED_TASK_EXECUTION_TIMESTAMP
        );
    ...
    private void defaultTaskExecutionShouldBeFound(String filter) throws Exception {
        restTaskExecutionMockMvc
            .perform(get("/api/task-executions?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(taskExecution.getId().intValue())))
            .andExpect(jsonPath("$.[*].taskExecutionTimestamp").value(hasItem(sameInstant(DEFAULT_TASK_EXECUTION_TIMESTAMP))))
        ...
        restTaskExecutionMockMvc
            .perform(get("/api/task-executions/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingTaskExecution() throws Exception {
        // Get the taskExecution
        restTaskExecutionMockMvc.perform(get("/api/task-executions/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void deleteTaskExecution() throws Exception {
        ...
        restTaskExecutionMockMvc
            .perform(delete("/api/task-executions/{id}", taskExecution.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
