...
public class TaskConfigResourceIT {
    ...
    public void getAllTaskConfigs() throws Exception {
        ...
        restTaskConfigMockMvc
            .perform(get("/api/task-configs?sort=id,desc"))
    ...
    public void getTaskConfig() throws Exception {
        ...
        // Get the taskConfig
        restTaskConfigMockMvc
            .perform(get("/api/task-configs/{id}", taskConfig.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    @Test
    @Transactional
    public void getAllTaskConfigsByConfigVersionContainsSomething() throws Exception {
        // Initialize the database
        taskConfigRepository.saveAndFlush(taskConfig);

        // Get all the taskConfigList where configVersion contains DEFAULT_CONFIG_VERSION
        defaultTaskConfigShouldBeFound("configVersion.contains=" + DEFAULT_CONFIG_VERSION);

        // Get all the taskConfigList where configVersion contains UPDATED_CONFIG_VERSION
        defaultTaskConfigShouldNotBeFound("configVersion.contains=" + UPDATED_CONFIG_VERSION);
    }

    @Test
    @Transactional
    public void getAllTaskConfigsByConfigVersionNotContainsSomething() throws Exception {
        // Initialize the database
        taskConfigRepository.saveAndFlush(taskConfig);

        // Get all the taskConfigList where configVersion does not contain DEFAULT_CONFIG_VERSION
        defaultTaskConfigShouldNotBeFound("configVersion.doesNotContain=" + DEFAULT_CONFIG_VERSION);

        // Get all the taskConfigList where configVersion does not contain UPDATED_CONFIG_VERSION
        defaultTaskConfigShouldBeFound("configVersion.doesNotContain=" + UPDATED_CONFIG_VERSION);
    }
    ...
    private void defaultTaskConfigShouldBeFound(String filter) throws Exception {
        restTaskConfigMockMvc
            .perform(get("/api/task-configs?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(taskConfig.getId().intValue())))
            .andExpect(jsonPath("$.[*].configKey").value(hasItem(DEFAULT_CONFIG_KEY)))
        ...
        restTaskConfigMockMvc
            .perform(get("/api/task-configs/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingTaskConfig() throws Exception {
        // Get the taskConfig
        restTaskConfigMockMvc.perform(get("/api/task-configs/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void deleteTaskConfig() throws Exception {
        ...
        restTaskConfigMockMvc
            .perform(delete("/api/task-configs/{id}", taskConfig.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
