...
public class JobResourceIT {
    ...
    public static Job createEntity(EntityManager em) {
        Job job = new Job().jobName(DEFAULT_JOB_NAME).jobStatusFlag(DEFAULT_JOB_STATUS_FLAG).createTimeStamp(DEFAULT_CREATE_TIME_STAMP);
        return job;
    }
    ...
    public static Job createUpdatedEntity(EntityManager em) {
        Job job = new Job().jobName(UPDATED_JOB_NAME).jobStatusFlag(UPDATED_JOB_STATUS_FLAG).createTimeStamp(UPDATED_CREATE_TIME_STAMP);
        return job;
    }
    ...
    public void createJob() throws Exception {
        ...
        // Create the Job
        restJobMockMvc
            .perform(post("/api/jobs").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(job)))
    ...
    private void defaultJobShouldBeFound(String filter) throws Exception {
        restJobMockMvc
            .perform(get("/api/jobs?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(job.getId().intValue())))
            .andExpect(jsonPath("$.[*].jobName").value(hasItem(DEFAULT_JOB_NAME)))
        ...
        restJobMockMvc
            .perform(get("/api/jobs/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void updateJob() throws Exception {
        ...
        updatedJob.jobName(UPDATED_JOB_NAME).jobStatusFlag(UPDATED_JOB_STATUS_FLAG).createTimeStamp(UPDATED_CREATE_TIME_STAMP);

        restJobMockMvc
            .perform(
                put("/api/jobs").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(updatedJob))
            )
            .andExpect(status().isOk());
    ...
    public void updateNonExistingJob() throws Exception {
        ...
        restJobMockMvc
            .perform(put("/api/jobs").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(job)))
            .andExpect(status().isBadRequest());
    ...
    public void deleteJob() throws Exception {
        ...
        // Delete the job
        restJobMockMvc
            .perform(delete("/api/jobs/{id}", job.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
