...
export class TaskExecutionConfigComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.taskExecutionConfigService
      ...
      .subscribe(
        (res: HttpResponse<ITaskExecutionConfig[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInTaskExecutionConfigs(): void {
    this.eventSubscriber = this.eventManager.subscribe('taskExecutionConfigListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITaskExecutionConfig[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/task-execution-config'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.taskExecutionConfigs = data || [];
