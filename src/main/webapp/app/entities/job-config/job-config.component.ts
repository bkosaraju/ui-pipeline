...
export class JobConfigComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.jobConfigService
      ...
      .subscribe(
        (res: HttpResponse<IJobConfig[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInJobConfigs(): void {
    this.eventSubscriber = this.eventManager.subscribe('jobConfigListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IJobConfig[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
